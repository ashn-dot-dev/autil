...
main(void)
{
    ...
    char* const mem = xalloc(NULL, size);
    ...
    // and triggering an out-of-memory condition in xalloc.
    // This behavior has been tested with the realloc implementation in glibc
    // and musl libc.
    xalloc(NULL, (size_t)-1);
#else
    puts("error: [xalloc] Out of memory");
    exit(EXIT_FAILURE);
#endif
